...
{
    public partial class secChangePsw : System.Web.UI.Page
    {
        ...
        {
            if (oldPsw.Text == string.Empty) { oldError.Text = "*"; }
            if (psw.Text == string.Empty) { pswError.Text = "*"; }
            if (cpsw.Text == string.Empty) { cpswError.Text = "*"; }
            ...
            try
            {
                using (niapsEntities1 ctx = new niapsEntities1())
                {
                    ...
                    var myp = ctx.adminUsers.Where(cp => cp.userName == userName && cp.role == rol).First();
                    if (ValidatePassword(password, myp.password) == true)
                    {
                        ...
                        {
                            msg.Text = "Password Confirmation does not match initial Password";
                        }
                    }
                    else { msg.Text = "old password is wrong"; }
